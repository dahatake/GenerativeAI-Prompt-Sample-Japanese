...
#### (Option) Step.1.2. (Option) マルチエージェントのグループチャットのサンプル
...
# 目的
- それぞれのサービスの実装に役に立つ追加のMicrosoft Azureのサービスを決定します。ユースケースと{サービス定義書}を高度に分析・解析して、{ガイドライン}を必ず参照して、どのサービスの、どの機能を、どのMicrosoft Azureのサービスを使用すべきかを決定してください。なぜ、そのMicrosoft Azureのサービスにしたのかの具体的で説得力のある説明も作成してください。
...
> ![IMPORTANT]
> APIなどAzureにDeploy済みの状態で、画面をここで作成予定。
...
- 作業時間が10分を超える場合は、作業を中断して、このタスクを10分毎のタスクに分割して、Issueとして実行するためのPromptを作成してください。それぞれのPromptを`work/screen-apicall-implementation-issue-prompt-<番号>.md`に日本語で追記してください。
