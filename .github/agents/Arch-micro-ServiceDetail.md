...
- 作業の進捗状況を、`work/{ユースケースID}/microservice-modeling-detail-work-status.md`に日本語で追記してください。

- 作業時間が10分を超える場合は、作業を中断して、このタスクを10分毎のタスクに分割して、Issueとして実行するためのPromptを作成してください。それぞれのPromptを`work/{ユースケースID}/microservice-modeling-detail-issue-prompt-<番号>.md`に日本語で追記してください。
